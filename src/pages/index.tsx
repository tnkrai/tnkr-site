...
const Home = ({ data }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const fallbackHeading = "Tnkr.ai is a tool for exploring the source knowledge of any technology." 
  const subheading = data?.attributes?.subheading;
  const fallbackSubheading = "We believe that every piece of technology, old or new, has a unique story within it. We want to help the next generation of tinkerers uncover these stories so new ones can be told.";
 
  
  const ctaButtonTitle = data?.attributes?.cta?.button?.title;
  const fallbackCtaButtonTitle = "Join Our Waitlist";
  const ctaButtonUrl = data?.attributes?.cta?.button?.url;
  const fallbackCtaButtonUrl = "https://tnkr.ai/waitlist";
  ...
  const fallbackBodyText = "As soon as we set off to build tinkr, our first instinct was to find a group of people who could benefit “the most” from having a tool like Tinkr. To ensure we were looking in the right places, we established 3 conditions for a given niche to be considered an ideal segment for us to plant our roots."; 
  const bodyList = data?.attributes?.landing_body?.body[1]?.children;

  const images = data?.attributes?.landing_body?.landing_images?.body_media?.data.map((image: 
    { attributes: { 
      url: string;
